...
let getProductData = async () => {
  try {
    ...
    let response = await fetch(`http://localhost:3000/demandforItem/${searchItem}`);
    let result = await response.json();
    // Extracting data from the result
    // Product names as labels
    const productName = "Product";
    let totalSales;
     totalSales= result.map(item=>{totalSales=+item.totalSales}); // Total sales as data values
     console.log(result)
    const data = {
